...
- **Command Name**: Имя команды в терминале (`aa`)
...
**Пользовательская история:** Как постоянный пользователь, я хочу установить aa глобально, чтобы использовать короткую команду `aa` вместо `uvx aa-cli`.

#### Критерии приемки

1. WHEN пользователь выполняет `uv tool install aa-cli` THEN Система SHALL установить пакет глобально
2. WHEN установка завершена THEN Команда `aa` SHALL быть доступна в PATH пользователя
3. WHEN пользователь выполняет `aa --version` THEN Система SHALL вывести версию без префикса uvx
...
2. WHEN пакет публикуется THEN Команда в терминале SHALL оставаться `aa`
3. WHEN пакет публикуется THEN Файл pyproject.toml SHALL содержать `name = "aa-cli"` и `[project.scripts] aa = "aa.cli:main"`
