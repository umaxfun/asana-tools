...
- **Человекочитаемый ID**: Короткий идентификатор формата CODE-N или CODE-N-M-..., где CODE - трехбуквенный код проекта, N, M - порядковые номера
...
**User Story:** Как пользователь, я хочу автоматически настроить конфигурацию с моими реальными проектами из Asana, чтобы не вводить данные вручную
...
2. WHEN Система получает токен от пользователя THEN Система SHALL выполнить запрос к Asana API для получения списка всех проектов пользователя
3. WHEN Система получает список проектов THEN Система SHALL предложить пользователю выбрать проекты для добавления в конфигурацию
4. WHEN пользователь выбирает проекты THEN Система SHALL запросить трехбуквенный код для каждого выбранного проекта с валидацией формата
5. WHEN Система собрала все данные THEN Система SHALL создать файл .aa.yml с реальным токеном и выбранными проектами с их кодами и asana_id
6. WHEN Система создает конфиг в интерактивном режиме THEN Система SHALL установить поле interactive в значение false
...
3. WHEN Система обрабатывает несколько проектов THEN Система SHALL использовать соответствующий трехбуквенный код для каждого проекта
